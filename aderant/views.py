...
def custom_logout(request):
    ...
    # Stocker le nom de l'utilisateur pour le message
...
def custom_login(request):
    ...
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            # Vérifier si c'est la première connexion (mot de passe temporaire)
...
def change_password_required(request):
    ...
    return render(request, 'registration/change_password_required.html', {
        'form': form
    })
...
def Accueil(request):
    ...
    # Calcul des nouveaux adhérents ce mois
    ...
    try:
        # Si vous avez un champ date_naissance
    ...
    evolution_nouveaux = round(
        ((nouveaux_mois - nouveaux_mois_dernier) / nouveaux_mois_dernier * 100) 
        if nouveaux_mois_dernier > 0 else 100, 
        1
    )
    
    total_mois_dernier = Adherant.objects.filter(date_integration__lt=debut_mois).count()
    evolution_total = round(
        ((total_adherants - total_mois_dernier) / total_mois_dernier * 100) 
        if total_mois_dernier > 0 else 0, 
        1
    )
    ...
    # Ajouter les statistiques admin si l'utilisateur est admin
    if request.user.is_superuser or request.user.is_staff:
        # CORRECTION : Utiliser CustomUser au lieu de User
        ...
        # Connexions ce mois
        connexions_mois = CustomUser.objects.filter(
            last_login__gte=debut_mois
        ).count()
        ...
        utilisateurs_recents = CustomUser.objects.filter(
            last_login__isnull=False
        ).order_by('-last_login')[:5]
        
        # Taux d'utilisation (approximatif)
...
def user_edit(request, user_id):
    ...
    if user == request.user:
     ...
     # Empêcher la modification des superutilisateurs
        ...
        if form.is_valid():
            ...
            if 'role' in form.changed_data:
                messages.success(request, 
                    f"✅ Rôle de {user.username} modifié : {form.cleaned_data['role']}"
                )
            else:
...
def calculer_branche(age):
    """Calcule la branche selon l'âge"""
    if age <= 10:
        return "Louveteau"
    elif age <= 15:
        return "Éclaireur"
...
def est_nouvel_adherant(date_integration):
    """Vérifie si l'adhérent a intégré il y a moins d'un mois"""
...
def AdherantListView(request):
     # Récupérer tous les adhérents avec leurs âges et branches calculés
    adherants = Adherant.objects.all()
    
    # Ajouter l'âge, la branche et le statut "nouveau" à chaque adhérent
    for adherant in adherants:
        ...
        # Déterminer si c'est un nouvel adhérent
    ...
    # CHEF_GROUPE a accès à tous les adhérents
...
def calculer_duree_adhesion(date_integration):
    """Calcule la durée d'adhésion en mois"""
    if not date_integration:
        return 0
    aujourd_hui = date.today()
...
def adherant_detail(request, adherant_id):
    ...
    # Préparer les compétences pour l'affichage
    competences_list = []
...
class AdherantCreateView(CreateView):
    ...
    success_url = reverse_lazy('adherant_list')  # Redirige vers la liste après ajout
...
def preinscription_list(request):
    ...
    return render(request, 'adherants/preinscription_list.html', {
